...
class ReposListCacheInfo200ResponseInnerGit(BaseModel):
    """
    ReposListCacheInfo200ResponseInnerGit
    """ # noqa: E501
    last_sync: datetime
    sync_status: StrictStr
    __properties: ClassVar[List[str]] = ["last_sync", "sync_status"]

    @field_validator('sync_status')
    def sync_status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['offline', 'inactive', 'in_sync', 'not_in_sync']):
            raise ValueError("must be one of enum values ('offline', 'inactive', 'in_sync', 'not_in_sync')")
        return value
