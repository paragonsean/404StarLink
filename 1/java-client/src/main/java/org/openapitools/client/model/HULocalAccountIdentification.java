...
public class HULocalAccountIdentification {
  ...
  private TypeEnum type = TypeEnum.HU_LOCAL;
   ...
   * @throws IOException if the JSON Element is invalid with respect to HULocalAccountIdentification
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!HULocalAccountIdentification.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!HULocalAccountIdentification.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `HULocalAccountIdentification` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
