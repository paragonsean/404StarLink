...
class UpdateSourceServerReplicationTypeRequest(BaseModel):
    ...
    """ # noqa: E501
    replication_type: StrictStr = Field(description="Replication type to which to update source server.", alias="replicationType")
    source_server_id: Annotated[str, Field(min_length=19, strict=True, max_length=19)] = Field(description="ID of source server on which to update replication type.", alias="sourceServerID")
    __properties: ClassVar[List[str]] = ["replicationType", "sourceServerID"]

    @field_validator('replication_type')
    def replication_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['AGENT_BASED', 'SNAPSHOT_SHIPPING']):
            raise ValueError("must be one of enum values ('AGENT_BASED', 'SNAPSHOT_SHIPPING')")
        return value
