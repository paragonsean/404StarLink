...
public class DatabaseRelatedChart {
   ...
   * @throws IOException if the JSON Element is invalid with respect to DatabaseRelatedChart
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DatabaseRelatedChart.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DatabaseRelatedChart is not found in the empty JSON string", DatabaseRelatedChart.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DatabaseRelatedChart.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DatabaseRelatedChart` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DatabaseRelatedChart.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DatabaseRelatedChart' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DatabaseRelatedChart> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DatabaseRelatedChart.class));

       return (TypeAdapter<T>) new TypeAdapter<DatabaseRelatedChart>() {
           @Override
           public void write(JsonWriter out, DatabaseRelatedChart value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DatabaseRelatedChart read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of DatabaseRelatedChart given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of DatabaseRelatedChart
   * @throws IOException if the JSON string is invalid with respect to DatabaseRelatedChart
   */
  public static DatabaseRelatedChart fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DatabaseRelatedChart.class);
  }

  /**
   * Convert an instance of DatabaseRelatedChart to an JSON string
   *
   * @return JSON string
   */
