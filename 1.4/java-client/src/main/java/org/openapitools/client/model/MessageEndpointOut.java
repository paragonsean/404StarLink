...
public class MessageEndpointOut {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(channels, createdAt, description, disabled, filterTypes, id, metadata, nextAttempt, rateLimit, status, uid, url, version);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to MessageEndpointOut
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!MessageEndpointOut.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MessageEndpointOut.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MessageEndpointOut` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
