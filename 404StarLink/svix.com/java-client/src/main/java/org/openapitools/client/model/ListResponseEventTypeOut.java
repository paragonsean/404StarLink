...
public class ListResponseEventTypeOut {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ListResponseEventTypeOut
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ListResponseEventTypeOut.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ListResponseEventTypeOut.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ListResponseEventTypeOut` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraydata.size(); i++) {
        EventTypeOut.validateJsonElement(jsonArraydata.get(i));
      };
