...
import org.openapitools.client.model.MessageAttemptEndpointOut;
...
public class ListResponseMessageAttemptEndpointOut {
   ...
   * @throws IOException if the JSON Element is invalid with respect to ListResponseMessageAttemptEndpointOut
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ListResponseMessageAttemptEndpointOut.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ListResponseMessageAttemptEndpointOut.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ListResponseMessageAttemptEndpointOut` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      for (int i = 0; i < jsonArraydata.size(); i++) {
        MessageAttemptEndpointOut.validateJsonElement(jsonArraydata.get(i));
      };
