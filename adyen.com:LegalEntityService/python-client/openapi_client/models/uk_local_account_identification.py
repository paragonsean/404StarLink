...
class UKLocalAccountIdentification(BaseModel):
    ...
    """ # noqa: E501
    account_number: Annotated[str, Field(min_length=8, strict=True, max_length=8)] = Field(description="The 8-digit bank account number, without separators or whitespace.", alias="accountNumber")
    sort_code: Annotated[str, Field(min_length=6, strict=True, max_length=6)] = Field(description="The 6-digit [sort code](https://en.wikipedia.org/wiki/Sort_code), without separators or whitespace.", alias="sortCode")
    type: StrictStr = Field(description="**ukLocal**")
    __properties: ClassVar[List[str]] = ["accountNumber", "sortCode", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['ukLocal']):
            raise ValueError("must be one of enum values ('ukLocal')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "type": obj.get("type") if obj.get("type") is not None else 'ukLocal'
        })
        return _obj
