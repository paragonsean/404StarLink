...
public class DayOfWeekRestriction {
   ...
   * @throws IOException if the JSON Element is invalid with respect to DayOfWeekRestriction
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DayOfWeekRestriction.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DayOfWeekRestriction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DayOfWeekRestriction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
