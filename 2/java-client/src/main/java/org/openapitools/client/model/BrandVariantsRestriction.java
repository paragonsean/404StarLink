...
public class BrandVariantsRestriction {
   ...
   * @throws IOException if the JSON Element is invalid with respect to BrandVariantsRestriction
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BrandVariantsRestriction.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BrandVariantsRestriction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BrandVariantsRestriction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
