...
import org.openapitools.client.model.StringMatch;
...
public class MerchantNamesRestriction {
   ...
   * @throws IOException if the JSON Element is invalid with respect to MerchantNamesRestriction
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!MerchantNamesRestriction.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MerchantNamesRestriction.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MerchantNamesRestriction` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("value") != null && !jsonObj.get("value").isJsonNull()) {
        ...
        if (jsonArrayvalue != null) {
          ...
          for (int i = 0; i < jsonArrayvalue.size(); i++) {
            StringMatch.validateJsonElement(jsonArrayvalue.get(i));
          };
        }
      }
  }
