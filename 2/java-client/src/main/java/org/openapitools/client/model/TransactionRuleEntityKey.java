...
public class TransactionRuleEntityKey {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TransactionRuleEntityKey
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TransactionRuleEntityKey.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TransactionRuleEntityKey.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TransactionRuleEntityKey` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("entityReference") != null && !jsonObj.get("entityReference").isJsonNull()) && !jsonObj.get("entityReference").isJsonPrimitive()) {
