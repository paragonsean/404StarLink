...
class EnableRule(BaseModel):
    ...
    groups: Optional[List[StrictStr]] = Field(default=None, description="DEPRECATED: Please use field `values`. Service group should have prefix `groups/`. The names of the service groups that are enabled (Not Implemented). go/predefined-service-groups. Example: `groups/googleServices`.")
