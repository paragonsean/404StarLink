...
**android_device_policy_tracks** | **List[str]** | The app tracks for Android Device Policy the device can access. The device receives the latest version among all accessible tracks. If no tracks are specified, then the device only uses the production track. | [optional] 
...
**applications** | [**List[ApplicationPolicy]**](ApplicationPolicy.md) | Policy applied to apps. | [optional] 
...
**choose_private_key_rules** | [**List[ChoosePrivateKeyRule]**](ChoosePrivateKeyRule.md) | Rules for determining apps&#39; access to private keys. See ChoosePrivateKeyRule for details. | [optional] 
