...
class ApplicationPolicy(BaseModel):
    ...
    delegated_scopes: Optional[List[StrictStr]] = Field(default=None, description="The scopes delegated to the app from Android Device Policy.", alias="delegatedScopes")
    ...
    def delegated_scopes_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['DELEGATED_SCOPE_UNSPECIFIED', 'CERT_INSTALL', 'MANAGED_CONFIGURATIONS', 'BLOCK_UNINSTALL', 'PERMISSION_GRANT', 'PACKAGE_ACCESS', 'ENABLE_SYSTEM_APP', 'NETWORK_ACTIVITY_LOGS', 'SECURITY_LOGS']):
                raise ValueError("each list item must be one of ('DELEGATED_SCOPE_UNSPECIFIED', 'CERT_INSTALL', 'MANAGED_CONFIGURATIONS', 'BLOCK_UNINSTALL', 'PERMISSION_GRANT', 'PACKAGE_ACCESS', 'ENABLE_SYSTEM_APP', 'NETWORK_ACTIVITY_LOGS', 'SECURITY_LOGS')")
        return value
