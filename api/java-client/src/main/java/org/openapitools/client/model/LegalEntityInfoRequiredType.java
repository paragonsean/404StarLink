...
public class LegalEntityInfoRequiredType {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(capabilities, entityAssociations, individual, organization, reference, type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to LegalEntityInfoRequiredType
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!LegalEntityInfoRequiredType.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!LegalEntityInfoRequiredType.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `LegalEntityInfoRequiredType` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
