...
public class PaymentInstrumentInfo {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(balanceAccountId, card, description, issuingCountryCode, paymentInstrumentGroupId, reference, status, type);
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to PaymentInstrumentInfo
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PaymentInstrumentInfo.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PaymentInstrumentInfo.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `PaymentInstrumentInfo` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
