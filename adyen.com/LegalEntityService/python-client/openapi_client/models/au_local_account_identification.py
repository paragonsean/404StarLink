...
class AULocalAccountIdentification(BaseModel):
    ...
    """ # noqa: E501
    account_number: Annotated[str, Field(min_length=5, strict=True, max_length=9)] = Field(description="The bank account number, without separators or whitespace.", alias="accountNumber")
    bsb_code: Annotated[str, Field(min_length=6, strict=True, max_length=6)] = Field(description="The 6-digit [Bank State Branch (BSB) code](https://en.wikipedia.org/wiki/Bank_state_branch), without separators or whitespace.", alias="bsbCode")
    type: StrictStr = Field(description="**auLocal**")
    __properties: ClassVar[List[str]] = ["accountNumber", "bsbCode", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['auLocal']):
            raise ValueError("must be one of enum values ('auLocal')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "type": obj.get("type") if obj.get("type") is not None else 'auLocal'
        })
        return _obj
