...
class CZLocalAccountIdentification(BaseModel):
    ...
    """ # noqa: E501
    account_number: Annotated[str, Field(min_length=2, strict=True, max_length=17)] = Field(description="The 2- to 16-digit bank account number (Číslo účtu) in the following format:  - The optional prefix (předčíslí).  - The required second part (základní část) which must be at least two non-zero digits.  Examples:  - **19-123457** (with prefix)  - **123457** (without prefix)  - **000019-0000123457** (with prefix, normalized)  - **000000-0000123457** (without prefix, normalized)", alias="accountNumber")
    bank_code: Annotated[str, Field(min_length=4, strict=True, max_length=4)] = Field(description="The 4-digit bank code (Kód banky), without separators or whitespace.", alias="bankCode")
    type: StrictStr = Field(description="**czLocal**")
    __properties: ClassVar[List[str]] = ["accountNumber", "bankCode", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['czLocal']):
            raise ValueError("must be one of enum values ('czLocal')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "type": obj.get("type") if obj.get("type") is not None else 'czLocal'
        })
        return _obj
