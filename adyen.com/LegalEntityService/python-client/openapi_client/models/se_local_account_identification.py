...
class SELocalAccountIdentification(BaseModel):
    ...
    """ # noqa: E501
    account_number: Annotated[str, Field(min_length=7, strict=True, max_length=10)] = Field(description="The 7- to 10-digit bank account number ([Bankkontonummer](https://sv.wikipedia.org/wiki/Bankkonto)), without the clearing number, separators, or whitespace.", alias="accountNumber")
    clearing_number: Annotated[str, Field(min_length=4, strict=True, max_length=5)] = Field(description="The 4- to 5-digit clearing number ([Clearingnummer](https://sv.wikipedia.org/wiki/Clearingnummer)), without separators or whitespace.", alias="clearingNumber")
    type: StrictStr = Field(description="**seLocal**")
    __properties: ClassVar[List[str]] = ["accountNumber", "clearingNumber", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['seLocal']):
            raise ValueError("must be one of enum values ('seLocal')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "clearingNumber": obj.get("clearingNumber"),
            "type": obj.get("type") if obj.get("type") is not None else 'seLocal'
        })
        return _obj
